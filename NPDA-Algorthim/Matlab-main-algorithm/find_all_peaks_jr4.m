...
if length(pks) > 0
    ...
    for i = 1:length(pks)
        x_pk = pos(leftbound-1+idx(i));
        y_pk = pks(i);
        pos1 = pos(leftbound-1+idx(i))-0.1;
        pos2 = pos(leftbound-1+idx(i))+0.1;
        [~, pos1_idx] = min(abs(pos1-pos));
        [~, pos2_idx] = min(abs(pos2-pos));
        data1 = data(pos1_idx);
        data2 = data(pos2_idx);
        a = (data2-data1)/(pos2-pos1);
        c = data1-a*pos1;
        b = -1;
        PCheight_alt(i) = y_pk-(a*x_pk+c);
        if PCheight_alt(i) < 0
            PCheight_alt(i) = 0;
        end
        %clf
    ...
    PCheight = PCheight_alt ./ ACheight;      % PC peak height normalised to AC peak height	
    ...
    for i = 1:length(pks)
        ...
        wt(i) = 1 - exp(-2.303 * PCheight(i) * 100);
