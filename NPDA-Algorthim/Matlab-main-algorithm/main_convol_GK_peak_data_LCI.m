function [AC_PC_thick, mean_med, pp, qq, weightedMean, wmSDE, mean_wt]=main_convol_GK_peak_data_LCI(data_filename, sheet,scan0,n,m,fit, plotFigures, peakFindType)
...
%nc = 0.7148;                % Reciprocal of cprneal RI
resolution=0.001;
...
%    file_path = [data_folder_path,'\',sorted_data_filename(a).name];
    ...
    % Read data
    dataq = dlmread(char(data_filename),',', 0, 1); 
    
    % Ensure arrays are of the same size
    dsize = size(dataq, 1);
    posq = dlmread(char(data_filename),',',[0 0 (dsize-1) 0]);
...
%       end
        ...
        amp(i, 1) = max(ya(1:spacer));
        amp(i, 2) = max(ya(spacer + 1:floor(4 / resolution)));
        ACq1(i, 1) = find(ya == amp(i, 1));
        ACq1(i, 2) = find(ya == amp(i, 2));
%        fprintf('%i\t%i\t%i\n', a , b, length(data_filename));
       ...
       if plotFigures == 1
        ...
        amp(i, 3) = max(y(1:spacer));
        amp(i, 4) = max(y(spacer + 1:floor(4 / resolution)));
        ACq1(i, 3) = find(y == amp(i, 3));
        ACq1(i, 4) = find(y == amp(i, 4));
%        if plotFigures == 1
%            figure(2);
%            hold on
%            plot(y);
...
%        end;
     ...
     %fprintf('%g\t%g\t%g\t%g\n', amp(i, 1), amp(i, 2), amp(i, 3), amp(i, 4));
     fprintf('%i\t%i\t%i\t%i\t', ACq1(i, 1), ACq1(i, 2), ACq1(i, 3), ACq1(i, 4));
        %%
        % 
        % # ITEM1
        % # ITEM2
        ...
        if indicator_num < 4.5
            ...
            peak_data(i, 2) = max_1;
            peak_data(i, 6) = max_2;
            [ql1, qr1] = find_AC_peak(ya(1:spacer));
            [ql2, qr2] = find_AC_peak(ya(spacer + 1:floor(4 / resolution)));
            peak_data(i, 3) = qr1 - ql1;
            peak_data(i, 7) = qr2 - ql2;
            peak_data(i, 4) = (qr1 + ql1 - 2 * peak_data(i, 1)) / peak_data(i, 3);
            peak_data(i, 8) = (qr2 + ql2 - 2 * (peak_data(i, 5) - spacer)) / peak_data(i, 7);
...
%fprintf('Median thickness: %3.1f ± %3.1f µm \n',median_t*1000, madse_t*1000);
    ...
    fprintf('%3.1f\t%3.1f\n',  mean_w * 1000, sd_w * 1000);
    verify_results(1) = indicator_num;
    verify_results(2) = mean_w * 1000;
    verify_results(3) = sd_w * 1000;
    verify_results(4) = mean_AC;
    verify_results(5) = mean_PC;
