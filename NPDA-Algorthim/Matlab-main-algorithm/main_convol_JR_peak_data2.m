...
resolution=0.001;
plotFigures = 0;            % Save time, cut out plots
trunc_low = 1200;
trunc_high = 2000;
...
% Create set of data points at 1um intervals by interpolation
    ...
    posq = posq';
    data0;
    dataq = round(interp1q(pos, data0, posq));
    posq = round(posq / resolution);
...
%       if m~=1
        [mean_data, ACp(i)] = Average_AC_align2(posq,data, resolution, 1);
...
%       end
        ...
        md = median(mean_data);
        ya = mean_data(trunc_low:trunc_high) - md;
        max_1 = max(ya(1:1400 - trunc_low));
        max_2 = max(ya(1401 - trunc_low:trunc_high - trunc_low));
...
%        fprintf('%i\t%i\t%i\n', a , b, length(data_filename));
        ...
        %fprintf('%i\t%i\t%i\t%i\n', max_1, max_2, size(ya, 1), size(y, 1));
        %max_1 = max(y(1:1400));
...
%        end;
       ...
       %ACpos(i)=findAC_quad(posq(q1:q2), y(q1:q2), resolution);           % AC position
        ...
        if indicator_num < 4.5
            ...
            peak_data(i, 2) = max_1;
            peak_data(i, 6) = max_2;
            [ql1, qr1] = find_AC_peak(ya(1:1400 - trunc_low));
            [ql2, qr2] = find_AC_peak(ya(1401 - trunc_low:trunc_high - trunc_low));
...
%fprintf('Median thickness: %3.1f ± %3.1f µm \n',median_t*1000, madse_t*1000);
    ...
    fprintf('%.3f\t%i\t%3.1f\t%3.1f\n', indicator_num, a, mean_w, sd_w);
    verify_results(a, 1) = indicator_num;
    verify_results(a, 2) = mean_w;
    verify_results(a, 3) = sd_w;
