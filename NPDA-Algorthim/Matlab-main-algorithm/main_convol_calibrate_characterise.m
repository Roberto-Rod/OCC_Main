function [ output_args ] = main_convol_calibrate_characterise( data_folder_path,sheet,scan0,n,m,fit, plotFigures, peakFindType )
%
...
for j = 1 : z
    ...
    if (datamax > 3500)
        ...
        %fprintf('%i\t%i\t%i\t%i\n', dataLH, dataLHgrad, dataRH, dataRHgrad);
        if (peak_replaced == 1) || (((dataLH < datamax / 2) || (dataLHgrad < 0)) && ((dataRH < datamax / 2) || (dataRHgrad > 0)))
            ...
            for i=3:n           % Loop along each block
                ...
                [PCpos(i), PCheight(i), weight(i)]=find_all_peaks_jr3(ACind, pos, y, 0, 0, nc, resolution, 0);
                %fprintf('%i\t%i\t%f\n', PCpos(i), PCheight(i), weight(i));
            end

            ACpa = nonzeros(ACp);
            fprintf('%i\t%f\t%f\t%f\n', j, mean(ACpos), mean(PCpos), mean(weight));
