function [ output_args ] = main_convol_calibrate3_profiles( data_folder_path,sheet,scan0,n,m,fit, plotFigures, peakFindType )
%
...
profiles = zeros(800, length(data_filename));
...
for j = 1 : length(data_filename)
    ...
    [datamax, p_max] = max(dataq(:, 3));
    datamax = datamax - datamed;
    dataLH = dataq(3, 3) -  datamed;
    dataLHgrad = dataq(8, 3) - dataq(3, 3);
    dataRH = dataq(length(dataq) - 3, 3) - datamed;
    dataRHgrad = dataq(length(dataq) - 3, 3) - dataq(length(dataq) - 8, 3);    dataRH = dataq(length(dataq) - 3, 3) - datamed;
    peak_replaced = 0;
    ...
    if (p_max > LH_zone)  % If peak at extreme left of scan, assume AC peak
        temp_data = dataq(1: p_max - LH_zone, 3) - datamed;
    ...
    if (datamax > 4000)
        ...
        if (peak_replaced == 1) || (((dataLH < datamax / 2) || (dataLHgrad < 0)) && ((dataRH < datamax / 2) || (dataRHgrad > 0)))
            ...
            for i=27:27           % Loop along each block
                ...
                ymax = max(y(1200:1300));
                profiles(:, j) = y(1201:2000) / ymax;
...
dist_file = strcat(data_folder_path, '\Convolved profiles.csv');
dlmwrite(dist_file, profiles);
