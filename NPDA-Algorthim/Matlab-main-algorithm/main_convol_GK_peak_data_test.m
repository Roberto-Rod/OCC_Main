function [AC_PC_thick, mean_med, pp, qq, weightedMean, wmSDE, mean_wt]=main_convol_GK_peak_data_test(data_filename, lookupname, sheet,scan0,n,m,fit, plotFigures, peakFindType)
...
%    file_path = [data_folder_path,'\',sorted_data_filename(a).name];
    ...
    % Read data
    data0 = dlmread(char(data_filename),',', 1, 0); 
...
%       end
        ...
        amp(i, 1) = max(ya(1:1400));
        amp(i, 2) = max(ya(1401:4000));
...
%        fprintf('%i\t%i\t%i\n', a , b, length(data_filename));
       ...
       if plotFigures == 1
        ...
        amp(i, 3) = max(y(1:1400));
        amp(i, 4) = max(y(1401:4000));
...
%        end;
     ...
     fprintf('%i\t%i\t%i\t%i\n', ACq1(i, 1), ACq1(i, 2), ACq1(i, 3), ACq1(i, 4));
        %%
        % 
        % # ITEM1
        % # ITEM2
