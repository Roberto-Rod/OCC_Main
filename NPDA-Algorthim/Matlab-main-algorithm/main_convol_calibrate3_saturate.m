function [ output_args ] = main_convol_calibrate3_saturate( data_folder_path,sheet,scan0,n,m,fit, plotFigures, peakFindType, s_level )
%
...
saturated = zeros(1, length(data_filename));
...
for j = 1 : length(data_filename)
    ...
    [dataq, saturated(j)] = saturate(dataq, s_level);
...
s = num2str(s_level);
dist_file = strcat(data_folder_path, '\Dist ', s, '.csv');
dlmwrite(dist_file, [xpos caldata' saturated']);
lookup_file = strcat(data_folder_path, '\Cal lookup ', s, '.csv');
