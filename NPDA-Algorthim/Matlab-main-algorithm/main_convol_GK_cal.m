function [AC_PC_thick, mean_med, pp, qq, weightedMean, wmSDE, mean_wt]=main_convol_GK_cal(filename,sheet,scan0,n,m,p, fit, plotFigures, peakFindType, cal, exp)
...
%    fprintf('%i\n', i);
    ...
    if m>=1
       [mean_data, ACp(i), mdn(i)] = Average_AC(posq,data, resolution, 1);
    ...
    ACh(i) = max(ya(q1:q2));
        ...
        if peakFindType == 0
            ...
            [PCpos(i), PCheight(i), weight(i)]=find_all_peaks_jr(ACind, posq, y, slope, 0, nc, resolution);
...
AC_PC_thick = [ACpt, ACht, ACpos, ACheight, PCpos, PCheight, mdn, thick, weight];
