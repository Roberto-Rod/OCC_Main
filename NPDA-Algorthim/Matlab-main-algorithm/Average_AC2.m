function [mean_data, ACp, mdn, backshift] = Average_AC2(pos,data0, resolution, align)
...
backshift = 0;
umres = 1000 * resolution;

for scan=1:nscans 
    data=data0(:,scan);    % Select a single scan
    current_dir = pwd;
    cd('../Matlab-scan-preprocessing');
    medData(scan) = median_uint16_t(data, length(data));
    cd(current_dir);

    data = data - medData(scan);  % Subtract off median to give common baseline (nominally zero).
    ...
    [q1, q2] = find_AC_peak2(data);
    temp_pos = pos(q1:q2);
    temp = data(q1:q2);
    fwhm(scan) = q2 - q1;         % AC peak FWHM
    maxpos = findAC_quad_jr2(temp_pos, temp, resolution);

    distfrommin = abs(pos - maxpos);
    idx_true(scan) = find(distfrommin == min(distfrommin), 1);
    %idx_true(scan) = find(abs(pos - maxpos) < 0.0005, 1); 
    ...
    p_data(:, scan) = newdata(1000:1500, scan);
end
...
ACp = mean(idx_true)/1000;
mdn = mean(medData);       % Mean of the median levels, i.e. the amount subtracted off
