function process_confocal_data(filepath, filename, min, max, num_scans)
...
r = strcat(filepath, '\Confocal peak.csv');
%fprintf('%s\n', r);
peak = csvread(r, 1, 0);
...
for i = min: max
    ...
    rawdata = csvread(t);
    x = rawdata(:, 1);
    xq = x(1):0.001:x(length(x));
    a = size(rawdata, 1);
    scandata = zeros(length(xq), num_scans);
    scandata(:, 1)= xq;
    convdata = zeros(length(xq), num_scans);
    convdata(:, 1)= xq;
    ...
    for j = 3:num_scans+1
        ...
        k = j - 1;
        scandata(:, k) = interp1(x, rawdata(:, j), xq);
        m = median(scandata(:, k));
        scandata(:, k) = scandata(:, k) - m;
        
        % Perform convolution
        convdata(:, k) = conv(scandata(:, k), peak(:, 2), 'same');
        plot(xq, convdata(:, k));
                
        % Find the peak positions
        b = 1;
        for n = 40:(size(convdata, 1) - 40)
            if convdata(n, k) > 2
                %fprintf('%i', n);
