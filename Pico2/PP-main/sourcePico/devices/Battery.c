...
#define MAX_VBOOST_OUT_OF_RANGE_EXCEPTION (50)  // maximum 50 retries 
...
static bool_t isBoostVoltageDead(uint32_t vBoostAdc);
...
static void logBatteryVoltage(const uint16_t refVoltageAdc);
...
ErrorCode_t Battery_CheckVoltageBounds(void)
{
  ...
  uint16_t boostVoltageAdc = (uint16_t) ADC_GetConversion(Boost_Voltage) & 0x03FF;
   ...
   */
  uint16_t referenceVoltageAdc = (uint16_t) ADC_GetConversion(channel_FVR) & 0x03FF;

  logBatteryVoltage(referenceVoltageAdc);
   ...
   */
  if (isBoostVoltageDead(vBoostAdc))
  {
      // Skip the retries if the boost voltage is dead
      voltageExceptionCount = 0;
      errorCode = EC_VOLTAGE_OUT_OF_RANGE_E;
  }
  else if (isBoostVoltageOutOfRange(referenceVoltageAdc, vBoostAdc))
  {
    if (voltageExceptionCount++ > MAX_VBOOST_OUT_OF_RANGE_EXCEPTION)
    {
      voltageExceptionCount = 0;
      errorCode = EC_VOLTAGE_OUT_OF_RANGE_E;
    }
...
static bool_t isBoostVoltageDead(uint32_t vBoostAdc)
{
  return ((bool_t) (DEAD_VBOOST_LIMIT > vBoostAdc));
}
