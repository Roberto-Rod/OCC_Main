 ...
 *   File        : test_HET_55.c
 *
 *   Description: Acceptance test for HET-55
 *
 *****************************************************************************/
/* ***************************************************************************
...
//The device detects a short button press event in THERAPY DELIVERY and alerts the user accordingly
...
{  
  ...
  // simulate 30 seconds of ticks in end of PUMP DWON and THERAPY DELIVERY
  // to ensure that the green LED has finished flashing 
  picoTestFrameworkIo.pumpActivateResult = EC_OK_E;
  ...
  //simulate 30 seconds of ticks in THERAPY DELIVERY where the target pressure is achieved,
  //the device does not detect a leak yet and stays in the same state, LEDs off. Detect a short
  //button press for the last 2 ticks
  picoTestFrameworkIo.pumpActivateResult = EC_OK_E;
  TherapyDeliveryTargetVacuumUpdate_Ignore();
  uint32_t count = 300;
  for(uint32_t i = 0; count > i; i++)
  {
    if ((count-2) == i)  // press the button 
    {
      PicoTestFramework_SetPushButton(TRUE);
    }
    PicoTestFramework_Tick();
  ...
  //simulate the next 19 ticks when the LEDs are activated according ( total 20 ) to
  //the relevant pattern for the device status alert
  PicoTestFramework_SetPushButton(FALSE);
  ...
  //simulate more tick and check the LEDs are off as the device status alert is over
  PicoTestFramework_SetPushButton(FALSE);
  for(uint32_t i = 0; 300 > i; i++)
  {
    PicoTestFramework_Tick();
