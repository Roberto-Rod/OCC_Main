...
/**
 * @brief Returns the index of the nearest to value in __npda_x0 starting searching from the specified position start_idx
 * 
...
inline int nearest_neighbour(fptype value, int start_idx) {
    ...
    for (i=start_idx; i<MAX_WINDOW_SIZE; i++) {
        fptype newDist = value - NPDA_X0(i);
        if (newDist >= 0) {
            if (newDist < dist) {
                dist = newDist;
                idx = i;
...
void interpolate_scan(void) {
    ...
    for (i=0, idx=0; i<__interpolated_size; i++) {
        idx = nearest_neighbour(NPDA_X(i), idx);
        if (idx != -1) {
