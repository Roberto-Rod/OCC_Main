...
def _perform_startup_and_scan(tune_data: PlaybackData, scan_data: PlaybackData):
    ...
    with AsvControl() as asv:
        ...
        while measurement_number <= consecutive_measurements:

            # Simulate a scan start LCD touch
            print("Sending start scan touch to screen. Measurement number "  + str(measurement_number))
            ...
            logger.info("_perform_startup_and_scan: executing the scanning sequence number '%d'" , measurement_number)
            BasicScanning().execute(asv, scan_data)

            measurement_number += 1
            # Wait to check the measurement
            time.sleep(10)

            # Simulate a scan start LCD touch
            print("Touching the scan button to enable scan start...")
            asv.touch_panel.touch("scan_button_touch")
            # # Send touch lifted message
            # asv.touch_panel.transmit_touch_lifted("scan_button_touch_lift_start", "scan_button_touch_lift_end")
            # time.sleep(5)
