...
static camera_callback_t gp_callback;
...
static void frame_ready_callback(
    camera_receiver_handle_t *h_camera, status_t status, void *p_data);

/**
 * @brief
 *   Function called from camera receiver when a new frame is ready
 * @param
 *   h_camera Camera receiver handle
 * @param
 *   status kStatus_Success expected
 * @param[in]
 *   p_data Pointer to user data 
 */
static void frame_ready_callback(
    camera_receiver_handle_t *h_camera, status_t status, void *p_data)
{
 ...
 *   Initialise camera
 * @param
 *   p_callback Pointer to frame ready callback function
 */
void CAMERA_initialise(camera_callback_t p_callback)
{
    camera_config_t cfg;
    ...
    CAMERA_RECEIVER_Init(&cameraReceiver, &cfg, frame_ready_callback, NULL);
