...
uint16_t FLASH_ReadWord(uint16_t flashAddr) {
    ...
    return ((uint16_t) ((((uint16_t) PMDATH) << 0x08U) | PMDATL));
}

/*lint -e{613} Suppress: Possible use of null pointer 'ramBuf' in [left/right] argument to operator '['. */
...
/*lint -e{613, 818} Suppress: (613) Possible use of null pointer 'flashWordArray' in [left/right] argument to operator '['. */
//                  Suppress: (818) Pointer parameter 'flashWordArray' could be declared as pointing to const. Do not want to alter interface in MCC function.*/
int8_t FLASH_WriteBlock(uint16_t writeAddr, uint16_t *flashWordArray) {
    ...
    for (i = 0; i < WRITE_FLASH_BLOCKSIZE; i++) {
        ...
        PMDATL = (flashWordArray[i] & 0x00FF);
