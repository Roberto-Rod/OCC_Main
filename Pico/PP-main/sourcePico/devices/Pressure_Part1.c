...
Parameters     :  uint16_t* pInternal - Pointer to the internal pressure 
                  uint16_t* pExternal - Pointer to the external pressure  
                  uint16_t* pDiff - Pointer to the difference between the pressures
...
ErrorCode_t GetVacuum(uint16_t* pInternal,uint16_t* pExternal, uint16_t* pDiff)
{
  ...
  if(EC_OK_E == result)
  {
    ...
    while(0 != retry_count)
    {
      ...
      if ((EC_INT_PRESSURE_SENSOR_NOT_READY_E == result) ||
